...
function runQualityAudit() {
  console.log('Running Dataset Quality Audit...');

  const workspaceRoot = path.join(__dirname, '..', '..', '..');
  
  const markdown = `# Editorial Dataset Quality Report
...
`;
  ...
  fs.writeFileSync(path.join(workspaceRoot, 'DATASET_QUALITY_REPORT.md'), markdown);
  console.log('Generated DATASET_QUALITY_REPORT.md');
}
