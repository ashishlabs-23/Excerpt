...
async function main() {
  ...
  total++;
  const fallbackPlan = (processor as any).buildCropExpression([], 200, 400);
  ...
  total++;
  const screenPlan = await (processor as any).processClip('sample.mp4', 'output.mp4', 0, 10, { content_type: 'screen_recording' }).catch(() => null);
