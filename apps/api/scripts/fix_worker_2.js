...
const replacementBlock = `    // ─── Step 4: Finalize ───────────────────────────────────────────
...
const startIndex = content.indexOf('// ─── Step 4: Finalize');
const fallbackStartIndex = content.indexOf('// â”€â”€ Step 4: Finalize');
const start = startIndex !== -1 ? startIndex : fallbackStartIndex;
const endIndex = content.indexOf('  } finally {');

if (start !== -1 && endIndex !== -1) {
  content = content.substring(0, start) + replacementBlock + content.substring(endIndex);
  fs.writeFileSync(targetPath, content, 'utf8');
  console.log('Fixed successfully');
} else {
  console.log('Could not find start or end index', { start, endIndex });
}
