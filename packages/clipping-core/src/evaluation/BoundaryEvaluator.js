"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.BoundaryEvaluator = void 0;
class BoundaryEvaluator {
    evaluate(expected, generated) {
        let totalError = 0;
        let matches = 0;
        const notes = [];
        const regressions = [];
        for (const exp of (expected || [])) {
            let closest = (generated || [])[0];
            let minDiff = Infinity;
            for (const gen of (generated || [])) {
                const diff = Math.abs((gen.start_time ?? gen.start) - exp.start_time) + Math.abs((gen.end_time ?? gen.end) - exp.end_time);
                if (diff < minDiff) {
                    minDiff = diff;
                    closest = gen;
                }
            }
            if (closest && minDiff < 30) {
                totalError += minDiff;
                matches++;
                const closestStart = closest.start_time ?? closest.start;
                if (Math.abs(closestStart - exp.start_time) < 1.0) {
                    notes.push(`Hook for expected clip at ${exp.start_time}s starts naturally.`);
                }
                else {
                    notes.push(`Hook for expected clip at ${exp.start_time}s is off by ${Math.abs(closestStart - exp.start_time).toFixed(1)}s.`);
                }
            }
        }
        if (matches === 0) {
            return {
                component: 'BoundaryEvaluator',
                score: 0,
                passed: false,
                regressions: ['No overlapping boundaries found with expected clips.'],
                notes: ['Failed to map any generated clips to expected clips.']
            };
        }
        const avgError = totalError / matches;
        const score = Math.max(0, 100 - (avgError * 10)); // 10s avg error = 0 score
        const passed = score >= 80;
        if (!passed) {
            regressions.push(`Boundary score dropped to ${score.toFixed(1)}% (Threshold: 80%)`);
        }
        return {
            component: 'BoundaryEvaluator',
            score: Number(score.toFixed(1)),
            passed,
            regressions,
            notes
        };
    }
}
exports.BoundaryEvaluator = BoundaryEvaluator;
