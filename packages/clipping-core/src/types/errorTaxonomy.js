...
exports.PipelineError = exports.ErrorCategory = void 0;
exports.getSuggestedFix = getSuggestedFix;
exports.classifyPipelineError = classifyPipelineError;
var ErrorCategory;
(function (ErrorCategory) {
    ...
    // Processing
    ErrorCategory["FFMPEG"] = "FFMPEG";
    ErrorCategory["TRANSCRIPTION"] = "TRANSCRIPTION";
...
class PipelineError extends Error {
    constructor(details) {
        ...
        this.clipId = details.clipId;
        this.durationMs = details.durationMs;
        this.attempt = details.attempt;
        this.rootCause = details.rootCause;
        this.timestamp = new Date().toISOString();
        this.timeoutType = details.timeoutType;
        this.metadata = details.metadata;
        this.suggestedFix = details.suggestedFix || getSuggestedFix(details.category, details.timeoutType);
        this.causeErr = details.causeErr;
    }
    toJSON() {
        return {
...
function getSuggestedFix(category, timeoutType) {
    if (timeoutType) {
        switch (timeoutType) {
            case 'api_timeout':
...
function classifyPipelineError(error, hint) {
    ...
    if (error instanceof Error) {
        ...
        const anyErr = error;
        if (anyErr.stderr)
            parts.push(String(anyErr.stderr));
        if (anyErr.code)
            parts.push(String(anyErr.code));
        if (anyErr.signal)
            parts.push(String(anyErr.signal));
    }
